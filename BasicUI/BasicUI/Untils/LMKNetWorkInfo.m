...
+ (nullable NSString *)currentIPAddress {
    ...
    if ([self connectedToWiFi] || [self connectedToWiFiIPv6]) {
        // WiFi is in use
        ...
        if (WiFiAddress == nil || WiFiAddress.length <= 0) {
            ...
            WiFiAddress = [self wiFiIPv6Address];
            if (WiFiAddress == nil || WiFiAddress.length <= 0)
            {
                return nil;
            }
        ...
        // Return Wifi address
        return WiFiAddress;
    } else if ([self connectedToCellNetwork] || [self connectedToCellNetworkIPv6]) {
        ...
        if (CellAddress == nil || CellAddress.length <= 0) {
            // Error, no address found
            CellAddress = [self cellIPv6Address];
            if (CellAddress == nil || CellAddress.length <= 0)
            {
                return nil;
            }
...
{
    // Check if we're connected to WiFi
    NSString *WiFiAddress = [self wiFiIPv6Address];
    // Check if the string is populated
    if (WiFiAddress == nil || WiFiAddress.length <= 0) {
        // Nothing found
        return false;
...
{
    // Check if we're connected to cell network
    NSString *CellAddress = [self cellIPv6Address];
    // Check if the string is populated
    if (CellAddress == nil || CellAddress.length <= 0) {
        // Nothing found
        return false;
