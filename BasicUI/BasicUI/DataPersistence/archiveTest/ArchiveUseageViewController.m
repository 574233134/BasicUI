...
- (IBAction)oneRead:(UIButton *)sender
{
    ...
    [self alertWithTitle:@"读取成功" message:[NSString stringWithFormat:@"sex:%@,name:%@,age:%u",person.sex,person.name,person.age]];
}
...
- (IBAction)manyRead:(UIButton *)sender
{
    ...
    [self alertWithTitle:@"读取成功" message:[NSString stringWithFormat:@"person 人数为：%u",array.count]];
    for (int i=0; i<array.count; i++)
    {
        Person *person = array[i];
        NSLog(@"name:%@,sex:%@,age:%u",person.name,person.sex,person.age);
    }
}
